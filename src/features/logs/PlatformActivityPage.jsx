...
import { SURFACE_FILTERS } from 'src/features/logs/clientSurface'
...
export default function PlatformActivityPage() {
  ...
  useEffect(() => {
    ...
    if (router.query.surface) setSurface(String(router.query.surface))
  }, [
    router.isReady,
  ...
  }, [page, pageSize, category, search, userId, fromDate, toDate, ipFilter, pathFilter, deviceFilter, surface])
  ...
  const applySurface = next => {
    setSurface(next)
    setPage(0)
    const q = { ...router.query }
    if (!next) delete q.surface
    else q.surface = next
    void router.replace({ pathname: router.pathname, query: q }, undefined, { shallow: true })
  }
              ...
              {SURFACE_FILTERS.map(s => (
                <Chip
                  ...
                  sx={{
                    ...
                    fontFamily: ops.mono,
                    fontSize: 11,
                    bgcolor: surface === s.value ? ops.softMint : ops.canvas,
                    color: surface === s.value ? ops.live : ops.body,
                    border: `1px solid ${surface === s.value ? ops.live : ops.hairline}`
                  }}
                />
              ))}
            </Stack>
