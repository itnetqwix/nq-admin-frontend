...
import { getOpsEventDetail, resolveOpsEvent, runOpsBackfill } from 'src/services/opsApi'
...
export default function OpsEventsTab({ hub = false }) {
  const router = useRouter()
  const dispatch = useAppDispatch()
  const { confirm, ConfirmDialog } = useAdminConfirm()
  const eventsState = useAppSelector(selectOpsEvents)
  const stats = useAppSelector(selectOpsStats)
  const rows = eventsState.items
  const loading = eventsState.loading
  const total = eventsState.total
  const page = Math.max(0, eventsState.page - 1)
  const pageSize = eventsState.limit
  const setPage = p => dispatch(setOpsEventPage({ page: p + 1 }))
  const setPageSize = size => dispatch(setOpsEventPage({ limit: size, page: 1 }))
  ...
  return (
    <AdminPageShell
      ...
      actions={
        <Stack direction='row' spacing={1} flexWrap='wrap' useFlexGap>
          <Chip component={Link} href='/apps/ops?tab=health' label='Platform health' clickable variant='outlined' size='small' />
