...
import { updateTrainerCommission } from 'src/services/userAdminApi'
...
export default function AddEditCommision({ handleClose, trainer_id, onSaved }) {
  const {
    control,
    handleSubmit,
    formState: { errors }
  ...
  const updateCommision = async data => {
    const payload = {
      trainer_id,
      commission: data.commission === '' || data.commission == null ? null : data.commission,
      surge_multiplier_cap_bps:
    ...
    try {
      await updateTrainerCommission(payload)
  ...
  return (
    <Box>
      <Box padding={'1rem'}>
        ...
        <Box paddingTop={'1rem'}>
          <form noValidate autoComplete='off' onSubmit={handleSubmit(updateCommision)}>
            <Grid container spacing={3}>
                  ...
                  render={({ field: { value, onChange, onBlur } }) => (
                    <TextField
                      ...
                      helperText={
                        ...
                        'Blank clears override and uses live Pricing default'
                      }
                    />
                  )}
                />
              ...
              <Grid item xs={12} sm={12}>
                <Box display='flex' gap={1} justifyContent='flex-end'>
                  <CustomButton onClick={handleClose} startIcon={<CancelPresentationIcon />} variant='outlined'>
                    Cancel
                  </CustomButton>
                  <CustomButton type='submit' startIcon={<CheckIcon />} variant='contained'>
                    Save
                  </CustomButton>
                </Box>
              </Grid>
