...
export default function Booking() {
  ...
  const { pushQuery } = useAdminListUrlSync({
    router,
    pathname: '/apps/booking',
    queryMap: { status: 'status' },
    onHydrate: values => {
      if (values.search) {
        setSearchInput(values.search)
        dispatch(setBookingSearch(values.search))
      }
      if (values.status) dispatch(setBookingFilters({ status: values.status }))
      if (values.page) dispatch(setBookingPage({ page: Number(values.page) }))
    }
  })
  ...
  const detailRow = useMemo(
    () => items.find(r => String(r._id || r.id) === String(detailId)) || null,
    [items, detailId]
  )

  const chipCounts = counts || { total: 0, booked: 0, confirmed: 0, completed: 0, canceled: 0, refund: 0 }

  const scheduleSearch = value => {
    setSearchInput(value)
    if (searchTimer.current) clearTimeout(searchTimer.current)
    searchTimer.current = setTimeout(() => {
      dispatch(setBookingSearch(value.trim()))
      pushQuery({ search: value.trim(), status: statusFilter, page: 1 })
    }, 400)
  }

  const setStatus = value => {
    dispatch(setBookingFilters({ status: value }))
    pushQuery({ search, status: value, page: 1 })
  }

  const reload = () => void dispatch(fetchBookings())

  const openDetail = id => {
    if (!id) return
    setDetailId(String(id))
    void router.replace(
      { pathname: '/apps/booking', query: { ...router.query, bookingId: String(id) } },
      undefined,
      { shallow: true }
    )
  }
    ...
    },
    {
      ...
      renderCell: p => (
        <Typography variant='body2'>
          {p.row.is_instant ? (p.row.instant_phase ? `Instant · ${p.row.instant_phase}` : 'Instant') : 'Scheduled'}
        </Typography>
      )
    },
    {
      ...
      renderCell: p => (
        <Box>
          <Typography variant='body2'>
            {formatOpsDateTime(p.row.start_time || p.row.booked_date, { withSeconds: false })}
          </Typography>
      ...
      >
        <Stack direction='row' spacing={1.5} useFlexGap flexWrap='wrap' sx={{ mb: 2 }}>
          <Box sx={{ flex: '1 1 140px', minWidth: 120 }}>
            <OpsMetricTile label='Sessions' value={String(chipCounts.total || 0)} hint='All time' />
          </Box>
          <Box sx={{ flex: '1 1 140px', minWidth: 120 }}>
            <OpsMetricTile
              label='Pending'
              value={String(chipCounts.booked || 0)}
              hint='Awaiting coach'
              tone='warn'
          ...
          <Box sx={{ flex: '1 1 140px', minWidth: 120 }}>
            <OpsMetricTile label='Canceled' value={String(chipCounts.canceled || 0)} hint='Need refund check' />
          </Box>
          <Box sx={{ flex: '1 1 140px', minWidth: 120 }}>
            <OpsMetricTile
              label='Refunds'
              value={String(chipCounts.refund || 0)}
              hint='Any refund status'
              tone='accent'
        ...
        <OpsSurfaceCard sx={{ p: 0, overflow: 'hidden' }}>
          <AdminPageSection>
            <AdminFilterBar
              ...
              onRefresh={reload}
              resultCount={total}
              helperText='Server-paginated. Click a row for the case drawer. Cancel requires a reason and starts the refund.'
            >
              {STATUS_CHIPS.map(s => (
                <FilterChip
                  key={s.value || 'all'}
                  ...
                  label={s.label}
                  count={s.value ? chipCounts[s.value] : chipCounts.total}
            ...
            <AdminGridContainer>
              <AdminDataGrid
                ...
                onPaginationModelChange={m => {
                  dispatch(setBookingPage({ page: m.page + 1, limit: m.pageSize }))
